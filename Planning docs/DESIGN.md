# Style Reference — Aditya Khadse Portfolio
> animated chalkboard in a design studio. A near-black wall, warm cream chalk, and five color-coded highlighters — one for each project discipline.

**Theme:** dark

This is the design system for Aditya Khadse's portfolio website: a near-black stage where massive cream type, thin outlined pill buttons, and individually color-coded category labels create a typographic showcase rather than a traditional brochure site. The system runs on a single warm cream surface color (#fffce1) against an almost-black background, with category words each wearing their own vivid hue (green for About/Core, orange for SVG/Motion, pink for Scroll/Interaction, violet/lilac for Web Apps, blue for UI/Systems) — color functions as taxonomy, not decoration. Typography is the hero: a single sans-serif (Mori) at six weights, pushing to 224px for the main headline with aggressive negative tracking and near-1.0 line-height, so words feel carved rather than laid out. Buttons are almost exclusively ghost-pills with 100px radius and hairline cream borders; there are no filled CTAs, which lets the gradient hero flourish and keeps every interactive element weightless.
...
| Off Black | `#191919` | `--color-off-black` | Alternative dark surface for nested panels, chatbot widget, and code blocks |
| Shockingly Green | `linear-gradient(114.41deg, #0ae448 20.74%, #abff84 65.5%)` | `--color-shockingly-green` | Green accent gradient for links, tags, and active states. Do not promote it to a solid primary CTA fill |
| Light Green | `#abff84` | `--color-light-green` | Green text accent for links, tags, and emphasized short phrases |
| Orangey | `#ff8709` | `--color-orangey` | SVG/Motion category label, orange-tool icon fills, gradient endpoint in Orange Crush |
| Pink | `#fec5fb` | `--color-pink` | Scroll/Interaction category label, decorative splashes, gradient endpoint in Summer Fair |
| Lilac | `#9d95ff` | `--color-lilac` | Web Apps/Engineering category label, thin illustrative strokes, gradient endpoint in Purple Haze |
| Blue | `#00bae2` | `--color-blue` | UI/Systems category label, gradient endpoint in Skyfall and Emerald City |
| Core Green | `#dfffd1` | `--color-core-green` | About/Core category label, subtle brand-tinted background washes for feature cards |
...
- **Role:** Single-family system. Mori weight 600 at 224px with lh 0.9 and -0.02em tracking is the hero display; the same family at weight 400 body sizes carries every paragraph, label, and nav item.
...
**Role:** Default interactive control — navigation entry points, secondary actions, case study links

Transparent background, #fffce1 text and 1px cream border, 100px border-radius, 15px vertical and 24px horizontal padding, Mori 18px weight 600 lh 1.05. Used for secondary actions (e.g. 'Explore SVG', 'View Live Project', 'Close Chat'). The 100px radius plus thin border gives a high-tech, minimal-control feel; never fill these with color.

### Ghost Nav Link
**Role:** Top navigation items, footer text links

No background, no border, #fffce1 or #7c7c6f text at 16px Mori 400 lh 1.15. Underline on hover via color shift to #fffce1. Nav row gap 6px, vertical padding 10px. Group spacing tight (6–16px) to keep the navigation bar compact and editorial.

### Gradient-Stroked CTA Pill
**Role:** Primary call-to-action — the 'Let's build' or contact trigger

Ghost button (transparent fill) with a 1.5–2px gradient border from #0ae448 to #abff84 along 114.41deg, cream text, 100px radius, 15px/24px padding. Implemented via the --color-core-button-gradient token on the border or as a border-image. This is the only chromatic control in the system; it carries the active green and reads as 'actionable' without violating the outlined-only rule.

### Borderless Icon Button
**Role:** Close, menu, utility toggle in the nav row

Fully round (50% radius), no background, cream icon, 0px padding. Used sparingly for icon-only controls like the mobile menu trigger or audio mute.

### Category Color Label
**Role:** Naming convention for each portfolio discipline — About, SVG/Motion, Scroll/Interaction, Web Apps, UI/Systems

Mori 19–24px weight 400, single-word, rendered in a discipline-specific hue:
- **Scroll/Interaction:** `#fec5fb` (Pink)
- **SVG/Motion:** `#ff8709` (Orange)
- **Web Apps/Engineering:** `#9d95ff` (Lilac/Violet)
- **UI/Systems:** `#00bae2` (Blue)
- **About/Core:** `#dfffd1` (Core Green)
Functions as the visual anchor for each section and project category. The color-to-discipline mapping is the site's signature taxonomy.

### Status Banner
**Role:** Top-of-page status notice (e.g. 'Currently open to new projects')
...
**Role:** The 224px personal wordmark/headline on the landing hero
...
**Role:** Section eyebrows: '{ About }', '{ Selected Work }', '{ Contact }', footer taglines
...
### Discipline Feature Block
**Role:** Major project category showcase — one per discipline

Two-column row inside the Work section: left side holds a large soft-rendered 3D-style shape in the discipline's accent color (with internal gradient and ambient lighting); right side holds the category label in its hue, a 34–44px cream subhead, body copy at 23px, and an outlined cream pill 'Explore' button. Divided from the next block by a 1px #42433d hairline that spans the section width.

### 3D Organic Illustration
**Role:** Decorative hero and project-section visual

Soft 3D shapes (pill, dome, liquid blob) rendered with multi-stop gradients — typically a discipline's accent color graduating into a lighter tint (e.g. pink-to-blue for Scroll, orange-to-amber for SVG). No drop shadows on the canvas; the shapes are lit from within via gradient. Containment is loose; they overlap adjacent type rather than respecting a frame.

### Footer
**Role:** Closing navigation and legal block

Off-black #191919 background, 1px #42433d top divider, multi-column nav with cream links at 16px Mori 400, generous 60–80px vertical padding. Includes Aditya Khadse's signature wordmark, link columns, and social/secondary nav. The footer shifts one surface step lighter than the page canvas, creating a subtle terminator.

### Project Showcase Card
**Role:** Grid item in the Work/Projects section grid

Near-black surface with 8px corner radius, cream heading at 24–33px, no visible border, and a contained 16:9 or 1:1 preview area showing high-fidelity screenshots/animations of the project. Sits in a 2–3 column grid with 24px gaps. Padding 24px on all sides; preview art overflows the card slightly to suggest motion.

## Do's and Don'ts

### Do
- Set body text and all primary UI in Mori weight 400 at 16–19px with line-height 1.15; this is the system's resting rhythm.
- Use the five-discipline color mapping (Green = About/Core, Orange = SVG/Motion, Pink = Scroll/Interaction, Violet/Lilac = Web Apps, Blue = UI/Systems) for every category label — never reuse a color for a different discipline.
...
- Push the hero display headline to 224px weight 600 with line-height 0.9 and -0.02em tracking; let it bleed to the viewport edge rather than centering it inside a max-width container.
...
- Place a 1px #42433d hairline divider between discipline showcase blocks, full section width, with no padding around it.
...
| 1 | Nested Panel | `#191919` | Footer, chatbot widget, and code-block backgrounds, one step lifted from the canvas |
| 2 | Cream Surface | `#fffce1` | Light surface used sparingly for callout cards or promotional panels |

## Elevation

- **Discipline Feature Illustration:** `none — depth comes from internal multi-stop gradients, not box-shadow`
...
- Accent (About/Core brand): #dfffd1 (Core Green) / #0ae448 (Active Green)
- Accent (discipline labels): #fec5fb Scroll, #ff8709 SVG, #9d95ff Web Apps, #00bae2 UI
- primary action: no distinct CTA color (use outlined-only or active green gradient stroke)
...
3. **Discipline Feature Block**: Two-column row at 1280px max-width, 80px vertical gap. Left column holds a 480px soft 3D dome with a linear gradient from #fec5fb to #00bae2. Right column starts with '{ Selected Work }' in 16px Mori 400 #fffce1, then 'Scroll' at 19px Mori 400 #fec5fb, then a 44px Mori 600 #fffce1 subhead, then a 23px Mori 400 #fffce1 body paragraph, then the outlined explore button.

4. **Category Label Pill**: A single word 'SVG' at 34px Mori 600 lh 1.0 in #ff8709, no background, no border. Functions as the section anchor — appears identically sized in both the section header and the corresponding nav item.
